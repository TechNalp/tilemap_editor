...
window.addEventListener('load', () => {
    ...
    let cell_count = {w: 100, h: 100};
    let tile_size = {w: 32, h: 32};

    console.log(cell_count.w * tile_size.w + "px");
    console.log(cell_count.h * tile_size.h + "px");
    ...
    // Move canvas container when dragging with middle mouse button
    let m_pos;
    ...
    function move_canvas(e) {
        ...
        // control canvas container position
        if (new_left < 0) {
            new_left = 0;
        }
        if (new_top < 0) {
        ...
        if (new_left > editor.clientWidth - canvas_width) {
            new_left = editor.clientWidth - canvas_width;
        }
        if (new_top > editor.clientHeight - canvas_height) {
            new_top = editor.clientHeight - canvas_height;
        }
...
<template>
    <div id="editor" class="d-flex h-100 flex-fill overflow-hidden">
